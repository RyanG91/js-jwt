...
const jwtSecret = 'doggo123' // Should come from ENV - environment
...
// Tell Passport to process JWT - Pretty standard code block for JWT.
passport.use(new PassportJwt.Strategy({
  jwtFromRequest: PassportJwt.ExtractJwt.fromAuthHeaderAsBearerToken(),
  secretOrKey: jwtSecret,
  algorithms: [jwtAlgorithm]
}, (payload, done) => {
  User.findById(payload.sub).then((user) => { //payload id same as mongo id.
    if (user) {
      done(null, user)
    } else {
      done(null, false)
